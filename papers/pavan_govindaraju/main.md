...
Here, $a(t)^T$ denotes the transpose of the adjoint state vector (a row vector representing how sensitive the loss is to perturbations in each state dimension), and $\frac{∂ f(y(t), t, \theta)}{∂ y}$ is the Jacobian matrix representing the partial derivatives of the vector field $f$ with respect to the state $y$. Solving this linear differential equation backwards in time enables gradient computation with constant memory cost, instead of storing every intermediate step in memory so that it can apply the chain rule in the backward pass.
